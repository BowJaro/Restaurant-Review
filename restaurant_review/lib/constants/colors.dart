import 'dart:ui';
import 'package:flutter/material.dart';

abstract class AppColors {
  static const Color transparent = Color(0x00000000);
  static const Color primary = Color(0xFFEF4F5F);
  static const Color primary2 = Color(0xFFFF5F50);
  static const Color primary3 = Color(0xFFFF4C36);
  static const Color black = Color(0xff000000);
  static const Color blue = Color(0xff2196f3);
  static const Color white = Color(0xffffffff);
  static const Color textBlack = Color(0xff050505);
  static const Color textGray = Color(0xffBEBFC0);
  static const Color shadowGray = Color(0xffBEBFC0);
  static const Color errorRed = Color(0xffD32F2F);
  static const Color rateYellow = Color(0xffFFC107);
  static const Color rateGray = Color(0xffBEBFC0);
  static const Color textRed = Color(0xFFED5D5D);
  static const Color textGray1 = Color(0xFF737373);
  static const Color textBlack1 = Color(0xFF4F4F4F);
  static const Color pageBgGray = Color(0xFFF3F2F7);
  static const Color dividerGray = Color(0xFFECECEC);
  static const Color backgroundGray = Color(0xFFEEEEEE);
  static const Color tabTextColor1 = Color(0xFFD4D4D4);
}
